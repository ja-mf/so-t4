public class Page
{
	...
	// refresco de los contadores de referencia
	public int updateRefCounter() {
		if (loaded() == false) 
			return -1;

		// si el bit de referencia es 1, hacer unsigned right shift
		// y poner un 1 en el bit mas significativo
		if (R > 0) {
			refCounter = (byte) (refCounter >>> 1);
			refCounter = (byte) (refCounter | 0b10000000);
		}
		else 
			refCounter = (byte) (refCounter >>> 1);
		return 1;
	...
	public int getRefCounter() {
		// como no existe unsigned, se comparan los valores absolutos
		return Math.abs(refCounter);
	}
}
